...
This notebook introduces the `KMeans` class, an unsupervised algorithm that aims to find groups within given data. For this algorithm, we first specify a *K* value, which determines the number of groups created from the feature data. Then, the algorithm randomly assigns each data point to one of the K clusters. Next, we find the center of each cluster. After we determine these centroids, we re-assign each data point to the closest centroid, which is calculated by using the Euclidean distance formula.  
...
A more detailed overview of K-Means clustering can be found below.
